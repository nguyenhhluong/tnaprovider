...
// ── bash -n syntax check (actual execution) ──────────────────
try {
  execFileSync("bash", ["-n", resolve(ROOT, "install.sh")], { stdio: "pipe" });
  chk("bash -n passes", true, true, true);
} catch (e) {
  chk("bash -n passes", false, true, e.stderr?.toString() || "syntax error");
}
...
chk("install.sh verifies HTTPS routes", script.includes('curl -sI "https://') && script.includes('for domain'), true, script.includes('curl -sI "https://'));
...
chk("install.sh installs Caddy via official apt repo", script.includes("dl.cloudsmith.io/public/caddy/stable/debian.deb.txt"), true, script.includes("dl.cloudsmith.io/public/caddy/stable/debian.deb.txt"));
...
chk("Blocker1: no npm install -g runs in dry-run", !script.includes('npm install -g'), false, script.includes('npm install -g'));

// Blocker 2: existing production with --yes requires --force-existing
...
chk("Blocker5: CF token missing uses die", script.includes('die "CLOUDFLARE_API_TOKEN is not set'), true, script.includes('die "CLOUDFLARE_API_TOKEN is not set'));
...
// New blocker checks
chk("NewBlocker: no hardcoded caddy_2.9.1_amd64.deb", !script.includes("caddy_2.9.1_amd64.deb"), false, script.includes("caddy_2.9.1_amd64.deb"));
chk("NewBlocker: no /tmp/caddy.deb raw binary fallback", !script.includes("/tmp/caddy.deb"), false, script.includes("/tmp/caddy.deb"));
chk("NewBlocker: no ufw --force reset", !script.includes("ufw --force reset"), false, script.includes("ufw --force reset"));
chk("NewBlocker: Caddy reload/restart uses die", script.includes('systemctl restart caddy || die "Caddy'), true, script.includes('systemctl restart caddy || die "Caddy'));
chk("NewBlocker: Caddy active check after reload", script.includes('systemctl is-active --quiet caddy || die'), true, script.includes('systemctl is-active --quiet caddy || die'));
chk("NewBlocker: tnaprovider active check uses die", script.includes('systemctl is-active --quiet tnaprovider || die'), true, script.includes('systemctl is-active --quiet tnaprovider || die'));
chk("NewBlocker: local app check uses die", script.includes('curl -fsS "http://127.0.0.1:${TNA_PORT}/" >/dev/null || die "Local app'), true, script.includes('curl -fsS "http://127.0.0.1:${TNA_PORT}/" >/dev/null || die'));
chk("NewBlocker: HTTPS retry logic exists", script.includes("for attempt in 1 2 3 4 5"), true, script.includes("for attempt in 1 2 3 4 5"));
chk("NewBlocker: dry-run does not run system-modifying commands", script.includes('npm install --no-audit --no-fund" || die "npm install failed"') && script.includes('$CLI_DRY_RUN; then'), true, true);
chk("NewBlocker: Caddy installed via official apt repo (not hardcoded deb)", script.includes("/etc/apt/sources.list.d/caddy-stable.list"), true, script.includes("/etc/apt/sources.list.d/caddy-stable.list"));
