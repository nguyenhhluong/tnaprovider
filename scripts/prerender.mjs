...
async function prerender() {
  ...
  try {
    await new Promise((resolve) => {
      server = app.listen(4173, () => {
        console.log("Prerender server running on http://localhost:4173");
        resolve(server);
      });
    });
  } catch (err) {
    console.error("Could not start prerender server:", err.message);
    process.exit(1);
  }
  ...
  try {
    ...
    console.warn("Playwright browser not available, skipping prerender:", err.message);
