...
import { acceptInvite } from "../utils/authApi";
import { UserPlus, CheckCircle2, AlertTriangle, Loader2, Eye, EyeOff } from "lucide-react";
...
export function AcceptInvite() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await acceptInvite(token, password, name.trim());
      setSuccess(true);
    } catch (err) {
      ...
      <div className="flex flex-col min-h-screen bg-brand-gray dark:bg-brand-darker">
        <div className="flex-1 flex items-center justify-center px-4 py-16">
          <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-8 md:p-10 text-center">
            ...
            <h1 className="text-2xl font-display font-bold text-brand-dark dark:text-white mb-2">Invalid Invite</h1>
      ...
      <div className="flex flex-col min-h-screen bg-brand-gray dark:bg-brand-darker">
        <div className="flex-1 flex items-center justify-center px-4 py-16">
          <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-8 md:p-10 text-center">
            ...
            <h1 className="text-2xl font-display font-bold text-brand-dark dark:text-white mb-2">Account Created</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Your account has been set up. You can now sign in.</p>
            <Link to="/login" className="inline-flex items-center gap-2 px-6 py-3 bg-brand-accent text-white rounded-xl font-semibold hover:bg-brand-accent-hover transition-colors">
              Sign In
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-brand-gray dark:bg-brand-darker">
      <SEO title="Accept Invite | TNA Provider Platform" description="Accept your invitation." canonical="https://tnaprovider.com.au/accept-invite" />
      <div className="flex-1 flex items-center justify-center px-4 py-16">
        <div className="w-full max-w-md">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-8 md:p-10">
            <div className="flex flex-col items-center mb-8">
              ...
              <h1 className="text-2xl font-display font-bold text-brand-dark dark:text-white">Accept Invitation</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Set up your account to get started.</p>
            </div>
